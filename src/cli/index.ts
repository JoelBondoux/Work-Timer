...
function getNpmGlobalPrefix(): string {
  ...
  if (prefixResult.status !== 0) {
    throw new Error(
      `npm prefix -g failed with status ${prefixResult.status}. stderr=[${prefixResult.stderr}] stdout=[${prefixResult.stdout}]`
    );
  }
...
type InstallPathDiagnostics = {
  ...
  exists: boolean;
  hasGit: boolean;
  looksLikeWorkTimer: boolean;
  originUrl?: string;
  dirty?: boolean;
};

function inspectInstallPath(pathToInspect: string): InstallPathDiagnostics {
...
function printPanel(title: string, bodyLines: string[], tone: string = ANSI.cyan): void {
...
mcpCmd
  ...
  .option(
    '--clients <ids>',
    'Comma-separated client ids: claude-desktop,cursor,vscode,vscode-insiders,claude-code,codex-cli,gemini-cli,chatgpt-desktop'
  )
  .option('--server-path <path>', 'Absolute path to MCP server.js (defaults to this Work-Timer install)')
  .action((opts: { clients?: string; serverPath?: string }) => {
    try {
      const serverPath = opts.serverPath ?? getBundledMcpServerPath();
      const requestedIds: McpClientId[] | null = opts.clients ? parseClientIds(opts.clients) : null;
      const targets = discoverMcpTargets().filter((target) =>
        requestedIds ? requestedIds.includes(target.id) : true
      );
      ...
      for (const target of targets) {
        if (target.kind === 'json') {
          if (!target.exists) {
            ...
            console.log(
              `  Fix: run \`work-timer mcp install --clients ${target.id} --create-missing\` and restart the client.`
            );
            continue;
          }
        ...
        if (target.kind === 'command') {
          ...
          if (result.status === 0) {
            ...
            console.log(
              `  Fix: install/update the client CLI, then run \`work-timer mcp install --clients ${target.id}\`.`
            );
          }
          continue;
        }
  ...
  .action((opts: {
    ...
    try {
      ...
      const results = targets.map((target) => {
        ...
        return {
          target,
          status: 'skipped-manual' as const,
