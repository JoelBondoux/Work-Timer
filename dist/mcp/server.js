...
import { createProject, updateProject, listProjects, getProjectByName, findSimilarProjects, renameProject, deleteProject, mergeProjects, getProjectDeleteImpact, getProjectMergeImpact, } from '../core/projects.js';
...
import { markInvoiced, markPaid, adjustSession, getSession } from '../core/sessions.js';
...
server.tool('timer_start', 'Start a timer for a project. Creates the project if it does not exist. If no exact match is found but similar project names exist, returns a warning listing them — unless confirm_new_project is true.', {
    ...
    confirm_new_project: z.boolean().optional().describe('Set to true to create a new project even when similar names already exist'),
    ...
    try {
        ...
        if (!confirm_new_project) {
            ...
            if (!exact) {
                ...
                if (similar.length > 0) {
                    ...
                    return textResult(`No project named "${project}" exists, but similar projects were found: ${nameList}.\n` +
        ...
        return textResult(`Timer started for "${session.project_name}" (session #${session.id})\nStarted at: ${utcDbToLocal(session.start_time)}`);
    }
    catch (e) {
        return errorResult(e.message);
    }
});
server.tool('timer_stop', 'Stop a running timer. If no project specified, stops the most recently started timer.', {
    ...
    try {
        ...
        return textResult(`Timer stopped for "${session.project_name}" (session #${session.id})\nDuration: ${formatDuration(durationMs / 60000)}`);
    }
    catch (e) {
        return errorResult(e.message);
    }
});
server.tool('timer_pause', 'Pause a running timer. If no project specified, pauses the most recently started running timer.', {
...
server.tool('timer_resume', 'Resume a paused timer. If no project specified, resumes the most recently paused timer.', {
    project: z.string().optional().describe('Project name (optional)'),
}, async ({ project }) => {
    try {
        const client = await getClient();
...
server.tool('timer_status', 'Show all currently running and paused timers with elapsed time.', {}, async () => {
    try {
        const client = await getClient();
...
// --- Project tools ---
server.tool('project_create', 'Create a new project with optional billing settings.', {
...
server.tool('project_update', 'Update a project\'s billing settings.', {
    name: z.string().describe('Project name'),
...
server.tool('project_rename', 'Rename a project.', {
...
server.tool('project_delete', 'Delete a project. Blocked if sessions exist unless force is true, in which case all sessions are permanently deleted.', {
...
server.tool('project_merge', 'Move all sessions from the source project into the target project, then delete the source project.', {
    ...
    try {
        ...
        return textResult(`Merged "${source}" into "${result.target.name}" (${result.sessionsMoved} session(s) moved).`);
    }
    catch (e) {
        return errorResult(e.message);
    }
...
// --- Billing/query tools ---
server.tool('time_query', 'Query time spent on projects with optional date range and project filters. Returns billing details.', {
...
server.tool('billing_summary', 'Calculate billing amounts with filters for unbilled or unpaid sessions.', {
    project: z.string().optional().describe('Filter by project name'),
...
// --- Session tools ---
server.tool('session_adjust', 'Adjust the start and/or end time of a session. Supply times in local time (YYYY-MM-DDTHH:MM:SS); they are automatically converted to UTC for storage.', {
    ...
    try {
        ...
        if (updated.end_time)
            lines.push(`  End:   ${utcDbToLocal(updated.end_time)}`);
...
// --- Invoice tools ---
server.tool('mark_invoiced', 'Mark completed sessions as invoiced.', {
...
server.tool('mark_paid', 'Mark completed sessions as paid.', {
...
// --- Export tools ---
server.tool('export_csv', 'Export billing data as CSV text.', {
    project: z.string().optional().describe('Filter by project name'),
...
server.tool('export_xlsx', 'Export billing data as a formatted Excel workbook. Returns the file path.', {
    project: z.string().optional().describe('Filter by project name'),
    from: z.string().optional().describe('Start date'),
    to: z.string().optional().describe('End date'),
...
server.tool('export_preset', 'Export billing data as CSV formatted for a specific accounting package.', {
    ...
    try {
        const client = await getClient();
        const csv = await exportPresetCsv(client, { projectName: project, from, to }, preset, { accountCode: account_code, taxType: tax_type, paymentTermsDays: payment_terms_days });
        if (output_path) {
            const { writeFileSync } = await import('node:fs');
...
// --- Settings tool ---
server.tool('settings_update', 'View or update global default settings (default_rate, default_currency, default_min_block_minutes).', {
    key: z
