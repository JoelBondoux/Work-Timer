...
export function formatRunningTimers(timers) {
    if (timers.length === 0) {
    ...
    for (const t of timers) {
        ...
        lines.push(`  #${t.session_id} ${safeProjectName}  ${formatDuration(t.elapsed_minutes)}  (started ${utcDbToLocal(t.start_time)})${status}${notes}`);
    }
    return lines.join('\n');
}
export function formatBillingRecords(records) {
    if (records.length === 0) {
        return 'No billing records found.';
    }
    const lines = ['Billing Records:'];
    lines.push('  ID    | Project              | Date       | Duration | Billed   | Amount      | Status');
    ...
    for (const r of records) {
        ...
        const status = [
            ...
            .filter(Boolean)
            .join(', ') || 'pending';
        lines.push(`  #${String(r.session_id).padEnd(4)} | ${safeProjectName.padEnd(20).slice(0, 20)} | ${r.date} | ${duration} | ${billed} | ${amount.padEnd(11)} | ${status}`);
    }
    return lines.join('\n');
}
export function formatProjectTotals(totals) {
    if (totals.size === 0) {
    ...
    for (const [, total] of totals) {
        ...
        lines.push(`  ${sanitizeTerminalText(total.project_name)}: ${hours}h billed — ${sanitizeTerminalText(total.currency)} ${total.total_amount.toFixed(2)}`);
    }
    return lines.join('\n');
}
export function formatProject(project, effective) {
