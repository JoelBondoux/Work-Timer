 ...
 */
export function utcDbToLocal(utcStr) {
 ...
 */
export function utcDbToLocalDate(utcStr) {
 ...
 */
export function localDateTimeToUtcDb(localStr) {
    if (/^\d{4}-\d{2}-\d{2}$/.test(localStr)) {
        throw new Error(`"${localStr}" is a date-only value. Please supply a full datetime, e.g. "${localStr}T09:00:00".`);
    }
 ...
 */
export function localToUtcRangeStart(localStr) {
 ...
 */
export function localToUtcRangeEnd(localStr) {
    const iso = /^\d{4}-\d{2}-\d{2}$/.test(localStr)
        ? localStr + 'T23:59:59'
        : localStr;
    return toUtcDbString(new Date(iso));
