...
function validateMinBlockMinutes(minBlockMinutes) {
...
function validateProjectNumericOptions(opts) {
    if (!opts) {
        return;
    }
...
export async function createProject(client, name, opts) {
...
export async function updateProject(client, nameOrId, updates) {
...
export async function getProject(client, nameOrId) {
...
export async function getOrCreateProject(client, name, opts) {
    const existing = await getProjectByName(client, name);
    if (existing)
        return existing;
    return createProject(client, name, opts);
}
export async function listProjects(client, includeArchived = false) {
...
export async function renameProject(client, oldName, newName) {
    const project = await getProjectByName(client, oldName);
    if (!project)
        throw new Error(`Project not found: "${oldName}"`);
...
export async function deleteProject(client, name, opts) {
    ...
    if (sessions.rows.length > 0 && !opts?.force) {
        throw new Error(`Project "${name}" has ${sessions.rows.length} session(s). Use --force to permanently delete it and all its sessions.`);
    }
...
export async function mergeProjects(client, sourceName, targetName) {
    ...
    if (!target)
        throw new Error(`Target project not found: "${targetName}"`);
...
export async function getProjectDeleteImpact(client, name) {
    const project = await getProjectByName(client, name);
    if (!project)
        throw new Error(`Project not found: "${name}"`);
    const active = await client.execute({
    ...
    const activeCount = Number(active.rows[0].c);
    const sessionCount = Number(sessions.rows[0].c);
    const pauseCount = Number(pauses.rows[0].c);
    return {
...
export async function getProjectMergeImpact(client, sourceName, targetName) {
    const source = await getProjectByName(client, sourceName);
    if (!source)
        throw new Error(`Source project not found: "${sourceName}"`);
    const target = await getProjectByName(client, targetName);
 ...
 */
export async function findSimilarProjects(client, name, threshold = 0.6) {
